...
import { isLanguage, readLanguage, saveLanguage, type Language } from "@/lib/i18n/config";
import { translate, type TranslationKey } from "@/lib/i18n/dictionary";
import { formatDate, formatMonth, formatNumber, formatTime, formatWeekday } from "@/lib/i18n/format";
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    document.title = language === "en" ? "TOGTMOL — Study with Bondook" : "Тогтмол — Бондооктой хамт суралцъя";
  }, [language]);
  useEffect(() => {
    const onStorage = (event: StorageEvent) => {
      if (event.key === "togtmol-language" && isLanguage(event.newValue)) setLanguageState(event.newValue);
    };
  ...
  const setLanguage = useCallback((next: Language) => {
    ...
    saveLanguage(next, typeof window === "undefined" ? undefined : window.localStorage);
  }, []);
