...
function useMusicController() {
  ...
  const onState = (state: number) => {
    ...
    if (state === 1) mark("playing");
      ...
      if (source?.kind === "video" && sources.length > 1) {
        next(1);
        return;
      }
      mark("stopped");
  ...
  const next = (direction: number) => {
    ...
    if (source) {
      ...
      const target =
        sources[(index + direction + sources.length) % sources.length];
      if (target.id === source.id) {
        try {
          youtube.current?.seekTo?.(0, true);
          if (actual.current === "playing") void adapter.current?.play();
        } catch {
          fail("Дууг дахин эхлүүлж чадсангүй.");
        }
        return;
      }
      select(target.id);
