...
To enable Mongolian voice input in Bondook, set **server-only** `CHIMEGE_API_TOKEN`. The authenticated-user allowlist is `CHIMEGE_ALLOWED_USER_IDS`; when empty, the existing `BONDOOK_AI_ALLOWED_USER_IDS` list is used. Voice transcription shares the existing per-user limit of 20 online requests per UTC day. Audio is sent to Chimege only after the user enables the in-chat consent checkbox and starts a recording; recordings are capped at 60 seconds, converted to mono 16 kHz WAV in the browser, and are not stored by Togtmol. The transcript is inserted into the editable chat field and is not sent to Bondook until the user submits it. Add the token as a Cloudflare Worker secret (for example, `npx wrangler secret put CHIMEGE_API_TOKEN`) and deploy; never put it in `NEXT_PUBLIC_*` or commit it.
