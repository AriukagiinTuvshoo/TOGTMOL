...
describe("calendar and statistics", () => {
  ...
  it("uses freeze reserves across missed days without consuming today's freeze", () => {
    ...
    expect(
      streakWithFreezes(new Set(["2026-09-14"]), "2026-09-15", 2),
    ).toMatchObject({
      streak: 1,
      freezesUsed: 0,
      freezesRemaining: 2,
  ...
  it("detects unfinished late-night planned Pomodoro sessions", () => {
    ...
    expect(result.unfinishedPercent).toBeCloseTo(66.6667, 3);
  });
